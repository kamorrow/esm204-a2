...
## Non-Linear
carbon_a_poly2 <- lm(abatement ~ poly(value, 2), data = carbon_a)
summary(carbon_a_poly2)

carbon_a_poly3 <- lm(abatement ~ poly(value, 3), data = carbon_a)
summary(carbon_a_poly3)
...
ggplot(data = carbon_a) +
  geom_point(aes(x = value,
                 y = abatement),
                 color = "dark red") +
  labs(subtitle = "Sector A",
       title = "Abatement",
       x = "Quantity (tons abated)",
       y = "Price ($/ton)",
       caption = "Kazia Mermel & Keene Morrow\nESM 204 Spring 2020") +
  theme_minimal()
...
# read in data
abatement <- read.csv("ESM204_HW2_Data.csv") %>% 
  clean_names()
...
```{r}
# Part 1
# Plot Each sector's marginal abatement cost estimates
# Sector A

ggplot(data = abatement,
       aes(
         x = abatement,
         y = mc_a)) +
  geom_point() +
  labs(
    ...
    title = "Marginal Cost of Carbon Abatement for Economic Sector A")
...
ggplot(data = abatement,
  ...
  labs(
    x = "Abatement (tons)",
    y = "Marginal Cost of Abatement ($/ton)",
    title = "Marginal Cost of Carbon Abatement for Economic Sector B")
...
ggplot(data = abatement,
  ...
  labs(
    x = "Abatement (tons)",
    y = "Marginal Cost of Abatement ($/ton)",
    title = "Marginal Cost of Carbon Abatement for Economic Sector C")
...
ggplot(data = abatement,
  ...
  labs(
    x = "Abatement (tons)",
    y = "Marginal Cost of Abatement ($/ton)",
    title = "Marginal Cost of Carbon Abatement for Economic Sector D")


# Compound figure for all four sectors

# Write down a plausible functional form for the marginal cost of abatement
```

```{r}
# Part 2
# Convert each marginal cost function from abatement to pollution
