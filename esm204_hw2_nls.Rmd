...
ggplot() +
  ...
  labs(y = "Cost ($/ton)",
         x = "Abatement (tons)",
         title = "Sector C",
         subtitle = "Smoothed 40 and Above",
         caption = "ESM 204 Spring 2020\nAnthony Luna, Kazia Mermel, & Keene Morrow") + 
    theme_minimal() +
    theme(plot.caption = element_text(hjust = 0, face = "italic"))
```
...
## Polynomial
### Second Order
carbon_c_40_lm_poly2 <- lm(mc ~ poly(abatement, 2),
                        data = carbon_c_40)

summary(carbon_c_40_lm_poly2)
# Multiple R-squared:  0.9245,	Adjusted R-squared:  0.8962   
AIC(carbon_c_40_lm_poly2) # 106.4456

### Third Order
carbon_c_40_lm_poly3 <- lm(mc ~ poly(abatement, 3),
                        data = carbon_c_40)

summary(carbon_c_40_lm_poly3)
# Multiple R-squared:  0.9245,	Adjusted R-squared:  0.8962
AIC(carbon_c_40_lm_poly3) # 107.0187
...
# estimate growth rate (r) to be = carbon_c_lm_$coefficients[2] ~ 1.13
# or carbon_c_40_lm$coefficients[2] ~ 1.39
...
carbon_c_nls <- nls(mc ~ K/(1 + A*exp(-r*abatement)),
                    ...
                    start = list(K = 160,
                                 A = 159,
                                 r = carbon_c_40_lm$coefficients[2]),
                    trace = TRUE)
# 
...
## Predicted values using the model for plotting
# ALL DATA (red)
new_df <- data.frame(abatement = carbon_c$abatement)

mc_predict <- predict(carbon_c_lm_poly2, newdata = new_df)

predict_carbon_c_lm <- data.frame(new_df, mc_predict)
...
ggplot()+
  geom_point(data = carbon_c,
             aes(x = abatement,
                 y = mc)) +
  geom_path(data = predict_carbon_c_lm,
            aes(x = abatement,
                y = mc_predict),
            color = "red") +
