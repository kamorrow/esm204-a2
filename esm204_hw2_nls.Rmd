...
```{r}
# Sector A
predict_carbon_a_demand <- data.frame(mc_predict = predict(carbon_a_lm,carbon_a), sector = "a",
                                   abatement = current_a-carbon_a$abatement)

# Sector B
predict_carbon_b_demand <- data.frame(mc_predict = predict(carbon_b_nls, current_b-carbon_b$abatement),
                                   sector = "b",
                                   abatement = current_b-carbon_b$abatement)

# Sector C
new_df <- data.frame(abatement = current_c-carbon_c_40$abatement)
...
predict_c_stage <- data.frame(abatement = seq(0, 30, by = 10),
                              mc_predict = rep(current_c - 0, 4),
                              sector = rep("c", 4))

predict_carbon_c_40_lm_demand <- predict_carbon_c_40_lm %>% 
  union(predict_c_stage) %>% 
  arrange(abatement)

# Sector D
predict_carbon_d_nls_demand <- data.frame(mc_predict = predict(carbon_d_nls, current_d-carbon_d$abatement),
                                   sector = "d",
                                   abatement = carbon_d$abatement)

predict_demand <- predict_carbon_a_demand %>% 
  rbind(predict_carbon_b_demand) %>%  
  rbind(predict_carbon_c_40_lm_demand) %>%  
  rbind(predict_carbon_d_nls_demand) %>% 
  add_column(type = "Demand")

predict_values <- predict_values %>% union(predict_demand)

ggplot(data = predict_values) +
  geom_point(aes(x = abatement, # changed from geom_path to see more
                y = mc_predict,
                color = type)) +
  facet_wrap(~ sector,
             scales = "free")
...
>>>>>>> fb44c9410fcdd43fe1d1e67496117b4e6f3803c2
...
carbon_c_fun_inv <- function(P){
  m <- as.numeric(carbon_c_40_lm$coefficients[2])
  b <- as.numeric(carbon_c_40_lm$coefficients[1])
  ifelse(P<=carbon_c_fun_demand(P),current_c-((P-b)/m),carbon_c_fun_demand(0))
}
...
ggplot() +
  ...
  geom_path(data=carbon_agg, aes(x=sector_b,y=mc),color="green")+
