...
Cap on carbon. Each sector (A, B, and C) must cut its carbon emissions by 100 tons (thus reducing total emissions from 600 down to 300).
...
The abatement cost for each sector (A, B, and C) was calculated by integrating the functions derived previously from 0 to 100 tons of carbon.  Total cost was calculated as the sum of all sectors.
...
Cap and trade. Each sector (A, B, C) is allocated carbon permits equal to their current emissions minus 100 (same as in (a) above), thus achieving the total reduction of 300. Then, these three sectors are allowed to trade with each other. You will need to derive the outcome of that trading.
...
Based off of these prices, Sectors A and B will only need `r a_cnt_demand` and `r b_cnt_demand` tons respectively. Sector C however has a demand of `r c_cnt_demand` tons, therefore they will purchase the remaining credits from Sectors A and B. 
